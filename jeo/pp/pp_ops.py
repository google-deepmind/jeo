...
Prefer not to use `*` or `**` in pp fn arguments, in order to allow programatic,
...
def get_skai_concat(key1="pre_image_png", key2="post_image_png", outkey="image",
                    axis=-1, pop_origs=True):
...
def get_concat(keys, outkey="image", axis=-1, pop_origs=True, broadcast=False):
  """Concatenates images across a given axis."""
...
def get_squeeze_dim(*keys, axis=0):
...
def get_reduce(keys, outkey, reduction="prod"):
...
def get_expand_dim(*keys, axis=0):
...
def get_ensure_4d(keys, extra_axis=0):
...
def get_invert_bool(keys: str | Sequence[str]) -> Callable[..., Any]:
...
def get_select_channels(key_channels, axis=-1):
...
def get_select_channels_by_name(key, channels, names, axis=-1):
...
def get_extract_channels(
    from_key: str, key_channels: Mapping[str, int | Sequence[int]], axis=-1
):
...
def get_add_meta_as_channel(meta_key, to_key="image"):
...
@pp_utils.InKeyOutKey()
def get_remap(mapping, default_value=0):
