...
"""AutoAugment and RandAugment policies for enhanced geo-image preprocessing.

AutoAugment Reference: https://arxiv.org/abs/1805.09501
RandAugment Reference: https://arxiv.org/abs/1909.13719

This code is forked from
third_party/py/big_vision/pp/autoaugment.py
...
from google3.third_party.tensorflow.contrib import image as contrib_image
...
AVAILABLE_OPS = ['AutoContrast', 'Brightness', 'Color', 'Contrast', 'Cutout',
                 'Equalize', 'Invert', 'Posterize', 'Rotate', 'Sharpness',
                 'ShearX', 'ShearY', 'Solarize', 'SolarizeAdd', 'TranslateX',
                 'TranslateY']
...
def cutout_randsize(image, min_size_ratio=0.1, max_size_ratio=0.6, replace=0):
  """Cutout with random cutbox size, aspect ratio and location."""
  h = tf.cast(tf.shape(image)[0], tf.float32)
  w = tf.cast(tf.shape(image)[1], tf.float32)
  pad_h = tf.random.uniform(
      [], minval=tf.cast(h * min_size_ratio / 2., tf.int32),
      maxval=tf.cast(h * max_size_ratio / 2. + 1, tf.int32),
      dtype=tf.int32)
  pad_w = tf.random.uniform(
      [], minval=tf.cast(w * min_size_ratio / 2., tf.int32),
      maxval=tf.cast(w * max_size_ratio / 2. + 1, tf.int32),
      dtype=tf.int32)
  return cutout(image, (pad_h, pad_w), replace)
...
def cutout(image, pad_size, replace=0):
  ...
  Args:
    ...
    pad_size: Specifies how big the zero mask that will be generated is that
      ...
      (2*pad_size x 2*pad_size). Can be tuple for different h/w sizes.
  ...
  w = tf.shape(image)[1]
  c = tf.shape(image)[2]
  if not isinstance(pad_size, (tuple, list)):
    pad_size = (pad_size, pad_size)

  # Sample the center location in the image where the zero mask will be applied.
  cutout_center_height = tf.random_uniform(shape=[], minval=0, maxval=h,
                                           dtype=tf.int32)

  cutout_center_width = tf.random_uniform(shape=[], minval=0, maxval=w,
                                          dtype=tf.int32)
...
def color(image, factor):
  ...
  degenerate = [
      tf.image.grayscale_to_rgb(tf.image.rgb_to_grayscale(image[:, :, i:i+3]))
      for i in range(0, num_channels, 3)]
  degenerate = tf.concat(degenerate, -1)
...
def contrast(image, factor):
  ...
  degenerate = [
      _degenerate(image[:, :, i:i+3]) for i in range(0, num_channels, 3)]
  degenerate = tf.concat(degenerate, -1)
  return blend(degenerate, image, factor)
...
def autocontrast(image):
  ...
  image = tf.stack([scale_channel(image[:, :, i])
...
def equalize(image):
  ...
  # Scales each channel independently and then stacks the result.
  image = tf.stack([scale_channel(image, i) for i in range(image.shape[-1])], 2)
  return image
...
def unwrap(image, replace):
  ...
  Args:
    image: A 3D Image Tensor with C+1 channels.
    replace: A one or C values 1D tensor to fill empty pixels.
  Returns:
    image: A 3D image Tensor with C channels.
  """
  ...
  image = tf.slice(image, [0, 0, 0], [image_shape[0], image_shape[1],
                                      image_shape[2]-1])
  return image
...
NAME_TO_FUNC = {
    ...
    'Cutout': cutout,
    'CutoutRandsize': cutout_randsize,
}
...
def level_to_arg(hparams):
  return {
      ...
      'CutoutRandsize': lambda level: (0.1*level/_MAX_LEVEL,
                                       0.6*level/_MAX_LEVEL),
...
def distort_image_with_randaugment(image, num_layers, magnitude,
                                   translate_const=10, cutout_const=20,
  ...
  Args:
    ...
    magnitude: Integer, shared magnitude across all augmentation operations.
      ...
      [5, 30]. Note that _MAX_LEVEL is set to 10 above.
    translate_const: Int value for translate ops (originally 100).
    cutout_const: Int value for cutout op (originally 40). If 0, then cutout
      is not included.
    ops: The subset of augmentation operations to choose from. Default to all.
  Returns:
    The augmented version of `image`.
  """
  assert image.dtype == tf.uint8, 'Expects not-normalized uint8 inputs.'
  replace_value = 128 + tf.zeros((tf.shape(image)[2]), dtype=image.dtype)
  tf.logging.info('Using randaug_geo preprocessing.')
  augmentation_hparams = HParams(
      cutout_const=cutout_const, translate_const=translate_const)
  ops = list(ops or AVAILABLE_OPS)
  if not cutout_const:
    ops.remove('Cutout')

  for layer_num in range(num_layers):
    op_to_select = tf.random_uniform([], maxval=len(ops), dtype=tf.int32)
